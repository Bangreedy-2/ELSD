...
This chapter details the formal grammar design of the GG-Code language. The language is designed to be deterministic, unambiguous, and readable, drawing inspiration from natural language commands while maintaining strict syntactic structure. The grammar is defined using a recursive descent approach, with clear separation between statement types and expression evaluation.

\section{Language Structure}
The top-level structure of a GG-Code program consists of a sequence of statements. The parser processes these statements sequentially, allowing for a straightforward execution model. Unlike free-form natural language processing, the grammar enforces a rigid structure where every command must begin with a reserved keyword, ensuring predictability and simplifying the compilation process.
...
Motion commands control the physical movement of the machine tool. The primary command is \texttt{Move}, which accepts target coordinates specified either by axis values (e.g., \texttt{X10 Y20}) or by named locations. The grammar also supports stopping and pausing operations, which are critical for multi-stage manufacturing processes.
